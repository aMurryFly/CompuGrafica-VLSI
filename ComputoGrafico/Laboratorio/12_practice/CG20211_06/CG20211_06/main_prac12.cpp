...
int main()
{
	...
	//Personaje mío
	ModelAnim murry("resources/objects/personajeMurry/Flair.dae");
	murry.initShaders(animShader.ID);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::translate(glm::mat4(1.0f), glm::vec3(60.3f, 1.75f, 0.3f)); // translate it down so it's at the center of the scene
		model = glm::scale(model, glm::vec3(0.1f));	// it's a bit too big for our scene, so scale it down
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		animShader.setMat4("model", model);
		murry.Draw(animShader);
