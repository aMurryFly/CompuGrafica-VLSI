...
void animate(void)
{
	...
	if (animacion)
	{
		...
		//Invirtiendo movimiento del carro
		...
		*/
		...
		/*
		//Agregando nuevo recorrido => "Recorrido 5"
		if (recorrido1)
		{
			movAuto_z -= 0.7f;
			orienta = 180.0f;
		...
		if (recorrido5) {
			//NOTA: m = dz 200 / dx 300
			movAuto_x += 0.666666f; //va más lento pero proporcional (TENER CUIDADO)
			movAuto_z += 1.0f;
			//NOTA: arctan(c.o 200 / c.a 300)
			orienta = 33.69f; 
			if (movAuto_z >= 150.0) {
				recorrido5 = false;
				recorrido1 = true;
		...
		*/
		...
		if (recorrido6) {
			movAuto_x += 1.75f; // m=0.75
			movAuto_z -= 1.0f;
			//NOTA: Sumamos 90 al arctan(150/200)=36.8698  
			orienta = 126.87f;  
			if (movAuto_x >= 0.0) {
				recorrido6 = false;
				recorrido1 = true;
